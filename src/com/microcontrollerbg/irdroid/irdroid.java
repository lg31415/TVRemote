...
250);
    ...
    //    Button info_play = (Button) findViewById(R.id.Button02);
        ...
        Button movie001 = (Button) findViewById(R.id.movie001);
        Button movie002 = (Button) findViewById(R.id.movie002);
        Button movie003 = (Button) findViewById(R.id.movie003);
        Button movie004 = (Button) findViewById(R.id.movie004);
        Button movie005 = (Button) findViewById(R.id.movie005);
       //   Button apple_menu = (Button) findViewById(R.id.apple_menu);
...
//mAudioManager.setVolumeControlStream(AudioManager.STREAM_MUSIC);
  ...
  //  final Spinner spinDevice = new Spinner(this);
        ...
        String savedValue= getRemote();
    	for(int i=0;i<spinDevice.getCount();i++)
    	                         if(savedValue.equals(spinDevice.getItemAtPosition(i).toString()))
    	                         {
    	                         spinDevice.setSelection(i);
    	                             break;
    	                         }
            ...
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                ...
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	if (spinDevice.getSelectedItem()==null || spinCommand.getSelectedItem() == null) {
    		        	Toast.makeText(getApplicationContext(), "请选择一个设备和键位", Toast.LENGTH_SHORT).show();
...
//SystemClock.uptimeMillis() + 200);
        ...
        movie001.setOnTouchListener(new OnTouchListener()
        {
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                int action = motionevent.getAction();
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	// String cmd=null;
                  	 try {
                  		 doGet("http://218.108.85.235:8055/?userid=111222333&ctname=/movie/bianxingjingang2-8M-H264-EAC3.ts");
                         sendSignal(gdevice, "KEY_7");
         				 try {
         				    Thread.sleep(1000);
         				   } catch (InterruptedException e) {
         				    // TODO Auto-generated catch block
        				...
        				sendSignal(gdevice, "KEY_1");                    //	Log.i("repeatBtn", "MotionEvent.ACTION_DOWN");
                      //  mHandler.removeCallbacks(mUpdateTask);
...
//SystemClock.uptimeMillis() + 200);
        ...
        movie002.setOnTouchListener(new OnTouchListener()
        {
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                int action = motionevent.getAction();
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	// String cmd=null;
                  	 try {
                  	  doGet("http://218.108.85.235:8055/?userid=111222333&ctname=/movie/mojingxianzongH264.ts");
          				sendSignal(gdevice, "KEY_7");
        				 try {
        				    Thread.sleep(1000);
        				   } catch (InterruptedException e) {
...
//SystemClock.uptimeMillis() + 200);
        ...
        movie003.setOnTouchListener(new OnTouchListener()
        {
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                int action = motionevent.getAction();
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	// String cmd=null;
                  	 try {
                  		doGet("http://218.108.85.235:8055/?userid=111222333&ctname=/movie/zhushenzhizhan.ts");
                        sendSignal(gdevice, "KEY_7");
        				 try {
        				    Thread.sleep(1000);
        				   } catch (InterruptedException e) {
...
//SystemClock.uptimeMillis() + 200);
        ...
        movie004.setOnTouchListener(new OnTouchListener()
        {
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                int action = motionevent.getAction();
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	// String cmd=null;
                  	 try {
                  		doGet( "http://218.108.85.235:8055/?userid=111222333&ctname=/movie/konglongxingqiu.ts");
          				sendSignal(gdevice, "KEY_7");
        				 try {
        				    Thread.sleep(1000);
        				   } catch (InterruptedException e) {
...
//SystemClock.uptimeMillis() + 200);
        ...
        movie005.setOnTouchListener(new OnTouchListener()
        {
            public boolean onTouch(View view, MotionEvent motionevent)
            {
                int action = motionevent.getAction();
                if (action == MotionEvent.ACTION_DOWN)
                {
                	...
                	// String cmd=null;
                  	 try {
                  		doGet( "http://218.108.85.235:8055/?userid=111222333&ctname=/movie/gongfu.ts");
          				sendSignal(gdevice, "KEY_7");
        				 try {
        				    Thread.sleep(1000);
        				   } catch (InterruptedException e) {
...
return null;
    ...
    public boolean parse (String config_file) {
    	...
    	if (!file.exists()) {
    		if (config_file != LIRCD_CONF_FILE)
    			Toast.makeText(getApplicationContext(), "选择的文件不存在", Toast.LENGTH_SHORT).show();
    		else
    			Toast.makeText(getApplicationContext(), "配置文件不存在，请升级数据库", Toast.LENGTH_SHORT).show();
    		//selectFile();
    		return false;
    	}
    	
    	if (lirc.parse(config_file) == 0) {
 			Toast.makeText(getApplicationContext(), "不能解析数据库", Toast.LENGTH_SHORT).show();
			//selectFile();
    		return false;
    	}
    ...
    public void updateDeviceList(){
        ...
        if (str == null){
 			Toast.makeText(getApplicationContext(), "I配置文件丢失", Toast.LENGTH_SHORT).show();
			//selectFile();
    		return;
        }
...
//	tv.append(device + ": " +cmd + " command sent\n");
    ...
    void deleteConfigFile() {
    	...
    	if (!file.exists()) 
   			Toast.makeText(getApplicationContext(), "无配置文件\n" +
   					"没有文件被删除", Toast.LENGTH_SHORT).show();
    	else
    		if (file.delete()){
...
commandList.clear();
    		//	selectFile();
    			...
    			Toast.makeText(getApplicationContext(), "无法删除该文件", Toast.LENGTH_SHORT).show();
    }
    
    
    
    public boolean onCreateOptionsMenu(Menu menu) {
        menu.add(0, 0, 0, "文件解析").setIcon(android.R.drawable.ic_menu_upload);
        menu.add(0, 1, 0, "清空配置").setIcon(android.R.drawable.ic_menu_delete);
        menu.add(0, 2, 0, "数据升级").setIcon(android.R.drawable.ic_input_add);
        menu.add(0, 3, 0, "键位发送").setIcon(android.R.drawable.arrow_up_float);
        menu.add(0, 4, 0, "关于").setIcon(android.R.drawable.ic_menu_help);
        return true;
    }
     ...
     */
     ...
     public String getRemote() {
    	 return mPrefs.getString("CurrentRemote", "Samsung");
     }
     /**
